...
# 使用示例
res_name=["data512","data768",'data1024','data1280','data2048']
for res in res_name:
    directory = f'/data/dailei/WHUGeoGen3/test/{res}'  # 替换为你的目录路径
    filenames = get_filenames_recursively(directory)
    for filename in filenames:
# directory = '/data/dailei/WHUGeoGen3/finetune/data512'  # 替换为你的目录路径
# filenames = get_filenames_recursively(directory)
# for filename in filenames:
        ...
        if "caption" in filename:
            ...
            if res=="data512" or res=="data768":
                # final_json['source']=seg_path[24:]
                # final_json['target']=img_path[24:]
                final_json['source']=seg_path[24:]
                final_json['target']=img_path[24:]
            else:
                final_json['source']=seg_path[25:]
                final_json['target']=img_path[25:]
            final_json['prompt']=caption+", "+tag
            json_file1=json.dumps(final_json)
            with open(f'{directory}_BJ_NY.json', 'a') as f1:
                f1.write(json_file1)
                f1.write("\n")

            final_json['prompt']=caption
            json_file2=json.dumps(final_json)
            with open(f'{directory}_BJ_NY_caption.json', 'a') as f2:
                f2.write(json_file2)
                f2.write("\n")
            final_json['prompt']=tag
            json_file3=json.dumps(final_json)
            with open(f'{directory}_BJ_NY_tag.json', 'a') as f3:
                f3.write(json_file3)
                f3.write("\n")
