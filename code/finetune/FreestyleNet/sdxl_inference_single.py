...
if __name__ == "__main__":
    ...
    ckpt_path='./models/checkpoint_checkpoint_BJ_NY_data512_512_1e-05_sdxl_bj_fintune_256_512_BJ_NY_data512_768_1e-05/epoch=4-step=224920.ckpt'
    ...
    for d in data_name:
        output_name = f"/home/root123/dailei/01Data/cig_results/{d}/"
        if not os.path.exists(output_name):
            os.makedirs(output_name)
        root_name = f'/home/root123/dailei/01Data/cig/{d}/'
        for file in os.listdir(root_name):
            if file.endswith(".jpg"):
                img = os.path.join(root_name, file)
                seg = img.replace('.jpg', '.png')
                caption = img.replace('.jpg', '.caption')
                with open(caption, 'r') as f1:
                    caption =  f1.readline().strip()
                txt = img.replace('.jpg', '.txt')
                with open(txt, 'r') as f2:
                    txt = f2.readline().strip()
                prompt = caption + ',' + txt

                detect_map = cv2.imread(seg)
                ...
                if RESOLUTION == 512:
                    ...
                    for sample in samples:
                        ...
                        cv2.imwrite(os.path.join(output_name, file), sample)
